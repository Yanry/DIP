...
for m = 1:n
    for i = 1:r
        bg_processed(i, 1:bg_paths(i, m), :) = bg_processed(i, 1:bg_paths(i, m), :);
        bg_processed(i, (bg_paths(i, m)+p+1):(c+m*p), :) = bg_processed(i, (bg_paths(i, m)+1):(c+m*p-p), :);
        for j = 1:p
            bg_processed(i, (bg_paths(i, m)+j), :) = bg_processed(i, bg_paths(i, m), :);
        end
        %bg_processed(i, (bg_paths(i, m)+1), :) = bg_processed(i, bg_paths(i, m), :);
    end
end

imshow(bg_processed);
imwrite(bg_processed, "../processed_images/bg_expanded.png")

...
ob_gray = rgb2gray(object);
ob_gray = ob_gray(:, 750:1500);
[r, c] = size(ob_gray);

ob_outline = my_sobel(ob_gray);
for i = 1:r
    for j = 1:c
        if (ob_outline(i, j) ~= 0)
            ob_outline(i, j) = 1;
...
%ob_outline_1 = ob_outline_1 - ob_outline;
% 
% for i = 1:r
%     for j = 1:c
%         if (ob_outline(i, j) ~= 1)
...
% end
imshow(outline_1)
imwrite(ob_outline_1, "../processed_images/fg_maskbg_nod.png")
%% my conv function
...
function [img_out, n, img_out_fill] = my_imfillbg(img)
    ...
    while (any(any(img_out_fill ~= img_out)))
        ...
        if (mod(n, 100) == 0) 
            name = sprintf("../processed_images/fg_mask/fg_maskbg_%d.png", n);
            imwrite(img_out_fill, name);
        end
    end
end
